...
>>>>>>> ff0fffe3a6bc418b393d6ac89090ed1d0bca0b99
```
...
The biggest current challenge we have with this data set is that all of the currency values are currently strings. Another challenge will be dealing with outliers. Some candidates received a much higher amount than the majority of candidates skewing the data.
...
ggplot(data=campaign %>% filter(total_con < 1e+07),
       mapping = aes(x=total_con, fill = Region)) +
  geom_boxplot() +
